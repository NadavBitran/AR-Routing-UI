...
/**
 * Custom hook for handling the validation of the routes and steps in the route list.
 *
 ...
 *
 * @returns {() => boolean} A function to initiate the validation of the route list. Return true if the route list is valid, false otherwise.
 */
export default function useRouteListValidation(routeList, validationActions) {
    const validateRouteList = () => {
        const unvalidatedRoutes = routeList.filter(
            (route, routeIndex) => !validateRouteProps(route, routeIndex)
        );
    ...
    const validateRouteProps = (route, routeIndex) => {
        ...
        if (!validationHelpers.isInputEnglish(route.name))
            errorMessage = 'Route name must be in English';
        if (validationHelpers.isInputEmpty(route.name))
            errorMessage = 'Route name is required';

        const isRouteValid = errorMessage === undefined;

        validationActions.updateRouteValidationStatusAt(
            routeIndex,
            'isNameValid',
            isRouteValid,
            errorMessage
        ...
        !isRouteValid &&
            validationActions.updateRouteExpansionStatusAt(routeIndex, true);
    ...
    const validateStepProps = (step, routeIndex, stepIndex) => {
        ...
        if (!validationHelpers.isInputPositiveNumberLessThan10000(step.length))
            errorMessage =
                'Step length must be a positive number between 0 and 10,000 meters!';

        const isStepValid = errorMessage === undefined;

        validationActions.updateStepValidationStatusAt(
            routeIndex,
            stepIndex,
            'isLengthValid',
            isStepValid,
